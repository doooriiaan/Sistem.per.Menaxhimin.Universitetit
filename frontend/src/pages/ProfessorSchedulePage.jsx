...
function ProfessorSchedulePage() {
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        setLoading(true);
        const response = await API.get("/profesor/orari");
        setSchedule(response.data);
        setError("");
      } catch (err) {
        setError(
          getApiErrorMessage(err, "Gabim gjate marrjes se orarit te profesorit.")
        );
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm text-slate-500">
        Seancat e mesimit per lendet ku je caktuar si profesor.
      </p>
